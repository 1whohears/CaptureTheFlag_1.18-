...
#track the thief
execute if score data ticks matches 0 if score blue flagState matches 2 as @e[tag=carryblue,limit=1] run function cf:game/blue/setflagpos
execute if score data ticks matches 0 if score red flagState matches 2 as @e[tag=carryred,limit=1] run function cf:game/red/setflagpos
execute if score data ticks matches 0 if score green flagState matches 2 as @e[tag=carrygreen,limit=1] run function cf:game/green/setflagpos
execute if score data ticks matches 0 if score yellow flagState matches 2 as @e[tag=carryyellow,limit=1] run function cf:game/yellow/setflagpos

#if the thief is killed put the flag back

#check if the flag has been captured

#thieves drop the flag if they disconnect
